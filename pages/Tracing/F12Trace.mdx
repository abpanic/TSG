---
title: Capture a browser trace
description: Capture network information from a browser trace to help troubleshoot issues.
ms.date: 05/01/2023
ms.topic: how-to
---
...
If you're troubleshooting an issue and seems like the issue is web app, you may want to first capture some additional information. For example, it can be helpful to share a browser trace, a step recording, and console output. This information can provide important details about what exactly is happening in the portal when your issue occurs.

> [!WARNING]
> Browser traces often contain sensitive information and might include authentication tokens linked to your identity. Please remove any sensitive information before sharing traces with others. Support uses these traces for troubleshooting purposes only.

You can capture this information any latest version of popular browser: Microsoft Edge, Google Chrome, Safari (on Mac), or Firefox. Steps for each browser are shown below.

## Microsoft Edge

The following steps show how to use the developer tools in Microsoft Edge. 

> [!NOTE]
> The screenshots below show the DevTools in Focus Mode with a vertical **Activity Bar**. Depending on your settings, your configuration may look different. 

1. Sign in to the web app. It's important to sign in _before_ you start the trace so that the trace doesn't contain sensitive information related to your account.
...
1. By default, the browser keeps trace information only for the page that's currently loaded. Set the following options so the browser keeps all trace information, even if your repro steps require going to more than one page.
    ...
    1. Select the **Console** tab, select **Console settings**, then select **Preserve Log**.
       ...
       ![Screenshot that highlights the Preserve log option on the Console tab in Edge.](./media/capture-browser-trace/edge-console-preserve-log.png)
    ...
    1. Select the **Network** tab. If that tab isn't visible, click the **More tools** (+) button and select **Network**. Then, from the **Network** tab, select **Preserve log**.
       ...
       ![Screenshot that highlights the Preserve log option on the Network tab in Edge.](./media/capture-browser-trace/edge-network-preserve-log.png)

1. On the **Network** tab, select **Stop recording network log** and **Clear**.

    ![Screenshot showing the Stop recording network log and Clear options on the Network tab in Edge.](./media/capture-browser-trace/edge-stop-clear-session.png)

1. Select **Record network log**, then reproduce the issue in the portal.

   ![Screenshot showing how to record the network log in Edge.](./media/capture-browser-trace/edge-start-session.png)

   You'll see session output similar to the following image.

   ![Screenshot showing session output in Edge.](./media/capture-browser-trace/edge-browser-trace-results.png) 
...
1. After you have reproduced the unexpected portal behavior, select **Stop recording network log**, then select **Export HAR** and save the file.
   ...
   ![Screenshot showing how to Export HAR on the Network tab in Edge.](./media/capture-browser-trace/edge-network-export-har.png) 
...
1. Back in the browser developer tools pane, select the **Console** tab. Right-click one of the messages, then select **Save as...**, and save the console output to a text file.
   ...
   ![Screenshot showing how to save the console output in Edge](./media/capture-browser-trace/edge-console-select.png) 

1. Package the browser trace HAR file, console output, and screen recording files in a compressed format such as .zip.

1. Share the compressed file with Support by using the **File upload** option in your support request
...
1. By default, the browser keeps trace information only for the page that's currently loaded. Set the following options so the browser keeps all trace information, even if your repro steps require going to more than one page:
    ...
    1. Select the **Console** tab, select the **Settings** icon, and then select **Persist Logs**.
       ...
       ![ ](./media/capture-browser-trace/firefox-console-persist-logs.png) alt-text="Screenshot of the Console setting for Persist Logs."

    1. Select the **Network** tab, select the **Settings** icon, and then select **Persist Logs**.

       ![ ](./media/capture-browser-trace/firefox-network-persist-logs.png) alt-text="Screenshot of the Network setting for Persist Logs."
...
1. Back in the browser developer tools pane, select the **Console** tab. Right-click one of the messages, then select **Save All Messages to File**, and save the console output to a text file.
   ...
   ![ ](./media/capture-browser-trace/firefox-console-select.png) alt-text="Screenshot of the Save All Messages to File command on the Console tab."
